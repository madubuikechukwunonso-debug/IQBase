...
export function calculateScore(
  ...
  answers.forEach(answer => {
    ...
    if (answer.isCorrect) {
      ...
      const timeBonus = getTimeBonus(answer.timeSpent, question.timeLimit * 1000) * basePoints
  ...
  const normalizedScore = config.baseScore + (accuracy * (config.maxScore - config.baseScore))
  ...
  const finalScore = Math.round(
    Math.max(config.minScore, 
      Math.min(config.maxScore, normalizedScore + variance)
    )
  )
  ...
  const categoryScores: CategoryScores = {
    logical: Math.round((categoryStats.logical.correct / Math.max(1, categoryStats.logical.total)) * 100),
    pattern: Math.round((categoryStats.pattern.correct / Math.max(1, categoryStats.pattern.total)) * 100),
    numerical: Math.round((categoryStats.numerical.correct / Math.max(1, categoryStats.numerical.total)) * 100),
...
function calculateSpeedScore(answers: Answer[], questions: Question[]): number {
  ...
  return Math.round(speedScores.reduce((a, b) => a + b, 0) / speedScores.length)
}
...
function generateRecommendations(
  ...
  if (weaknesses.includes('Logical Reasoning')) {
    recommendations.push('Practice syllogisms and logical deduction puzzles to strengthen your reasoning skills.')
  }

  if (weaknesses.includes('Pattern Recognition')) {
    recommendations.push('Engage in activities like Sudoku, chess, or sequence puzzles to improve pattern detection.')
  }

  if (weaknesses.includes('Numerical Reasoning')) {
    recommendations.push('Work on mental math exercises and percentage calculations to boost numerical fluency.')
  }

  if (weaknesses.includes('Processing Speed')) {
    recommendations.push('Try timed quizzes and quick-response games to increase your cognitive processing speed.')
  }

  // Add general recommendations
  if (recommendations.length === 0) {
    recommendations.push('Excellent performance! Challenge yourself with more complex problems to continue growing.')
  }

  recommendations.push('Regular cognitive exercise, adequate sleep, and a healthy diet support optimal brain function.')
  recommendations.push('Consider exploring new learning domains to diversify your cognitive abilities.')
...
export function getDetailedBreakdown(
  ...
  return {
    ...
    slowestCategory: categoryAverages[categoryAverages.length - 1]?.type || 'N/A'
  }
}
